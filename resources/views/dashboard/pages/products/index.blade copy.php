                                ...
                                <tbody>
                                    @forelse($products as $product)
                                        <tr>
                                            ...
                                            why this thing is not wrok on pagination why basically pagination page two it work fine but i move to next pagination it not owrkir nor modal open or any thing chnage in db 
                                            <td>
                                                <label class="switch">
...
<!-- 
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.special-offer-toggle').forEach(toggle => {
                toggle.addEventListener('change', function () {
                    ...
                    if (this.checked) {
                        ...
                        document.getElementById('price_before').value = this.dataset.before || '';
                        document.getElementById('price_after').value = this.dataset.after || '';

                        $('#specialOfferModal').modal('show');
                    } else {
                        // If OFF → turn off in database immediately
                        updateSpecialOffer(this.dataset.id, 0, null, null);
                    }
                });
            });
            ...
            document.querySelectorAll('.popular-toggle').forEach(toggle => {
                toggle.addEventListener('change', function () {
                    ...
                    fetch(`/products/${id}/popular-toggle`, {
                        ...
                        .then(res => res.json())
                        .then(data => {
                            console.log("Popular updated:", data);
                        })
